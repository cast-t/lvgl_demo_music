...
//random array, used to display dynamic effect of center spectrums when this demo starts running
...
lv_obj_t * _lv_demo_music_main_create(lv_obj_t * parent)
{
    ...
    lv_obj_t * cont = create_cont(parent);          //主容器创建
    create_wave_images(cont);                       //边缘波浪装饰创建
    lv_obj_t * title_box = create_title_box(cont);  //标题（曲名、作曲家等信息）创建
    lv_obj_t * icon_box = create_icon_box(cont);    //收藏、下载等图标创建
    lv_obj_t * ctrl_box = create_ctrl_box(cont);    //上一曲、下一曲、播放暂停等图标创建
    spectrum_obj = create_spectrum_obj(cont);       //乐曲播放动态图
    lv_obj_t * handle_box = create_handle(cont);    //下部装饰
...
# endif
    ...
    sec_counter_timer = lv_timer_create(timer_cb, 1000, NULL);  //播放音乐秒计时器
    lv_timer_pause(sec_counter_timer);

    /*Animate in the content after the intro time*/
    lv_anim_t a;

    start_anim = true;  //开启动画的标志

    stop_start_anim_timer = lv_timer_create(stop_start_anim_timer_cb, INTRO_TIME + 6000, NULL); //停止开启动画计时器
...
static lv_obj_t * create_cont(lv_obj_t * parent)
{
    ...
    //main_cont作为最大的容器，在未将LV_DEMO_MUSIC_SQUARE或LV_DEMO_MUSIC_ROUND置1的情况下
    //其中包含从上到下依次包含player, placeholder1, placeholder2三个子容器。将main_cont的子容器设为可滚动
    //且捕捉中部的情况下，下滑动作发生时，placeholder1自动置于中部；上滑动作发生时，player自动置于中部
    //。又因为placeholder1和placeholder2均清除了可点击标签，所以经过以上这些处理可以达到下滑隐藏播放界面，上滑
    //显示播放界面，且自动对齐的效果
...
static lv_obj_t * create_ctrl_box(lv_obj_t * parent)
{
    ...
    LV_IMG_DECLARE(img_lv_demo_music_btn_rnd);  //random play icon
    ...
    lv_obj_add_flag(play_obj, LV_OBJ_FLAG_CHECKABLE);   //Toggle checked state when the object is clicked
...
static void spectrum_draw_event_cb(lv_event_t * e)
{
    ...
    if(code == LV_EVENT_REFR_EXT_DRAW_SIZE) {   //如果需要的空间不足，获取额外绘图区域
#if LV_DEMO_MUSIC_LANDSCAPE
